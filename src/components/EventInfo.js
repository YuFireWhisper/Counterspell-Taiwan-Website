...
const EventInfoItem = styled.div`
  ...
  max-width: 400px;
  background: ${(props) =>
    props.itemBgColor || "#FFE5D9"}; /* 使用新配色方案的淡珊瑚橙 */
  margin: 10px;
  text-align: center;
  color: #333333; /* 深灰色文字 */
  border-radius: 20px;
...
const EventInfo = () => {
  ...
  return (
    <Section ref={sectionRef} bgColor="#E6E6FF">
      {" "}
      {/* 淡紫羅蘭色背景 */}
      <SectionTitle>
        <MapPin /> 活動資訊
      ...
      <EventInfoContainer>
        {/* 活動日期 */}
        <SectionAnimation>
          <EventInfoItem isVisible={isVisible} itemBgColor="#FFE5D9">
            {" "}
            {/* 淡珊瑚橙 */}
            <Calendar size={40} />
            <h3>活動日期</h3>
        ...
        <SectionAnimation>
          <EventInfoItem isVisible={isVisible} itemBgColor="#FFE5D9">
            ...
            <h3>活動地點</h3>
            <p>
              {content.eventDetails?.location || "台北市信義區信義路五段7號"}
            </p>
          </EventInfoItem>
        </SectionAnimation>
